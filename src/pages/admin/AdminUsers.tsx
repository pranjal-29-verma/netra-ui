              ...
              {canAssign && (
                <div>
                  ...
                  {rolesLoading ? (
                    ...
                    <div className="space-y-2">
                      {allRoles.map((role: AdminRole) => (
                        <label
                          key={role.id}
                          className={`flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition-colors ${
                            resolvedIds.has(role.id)
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={resolvedIds.has(role.id)}
          ...
          <div className="flex gap-2 p-5 border-t border-gray-100 dark:border-gray-700 flex-shrink-0">
            ...
            {canDelete && (
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 py-2 text-sm font-medium rounded-lg bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors disabled:opacity-50"
              >
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{data ? `${data.total} total` : '—'}</p>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                data?.users.map((u) => {
                  ...
                  return (
                    <tr key={u.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                      <td className="px-4 py-3">
                        <button onClick={() => setSelectedId(u.id)} className="flex items-center gap-3 text-left">
        ...
        {data && data.pages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 dark:border-gray-700">
            <p className="text-xs text-gray-500 dark:text-gray-400">Page {data.page} of {data.pages}</p>
